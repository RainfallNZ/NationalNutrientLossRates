...
CatchmentOutputData <- data.frame(NULL)
...
for (WaterQualitySite in Catchments$sID ){
  ...
  #Use the look up table to define TP and TN for each land use
...
#Ditch the geometry to make further processing easier
  ...
  CatchmentOutputData <- dplyr::bind_rows(CatchmentOutputData,CatchmentWide)
  #write to a csv file
...
write.table(CatchmentOutputData,file=file.path(ProjectDataDirectory,"Output.csv"),sep=",",row.names = FALSE,append=TRUE,quote = FALSE)
```
